...
describe('The account validation', () => {
  it.each(VALID_EMAILS)(`Should check email: %s is valid`,
      (validEmail)=>
        expect(validateEmail(validEmail)).toBe(undefined),
  );
  it.each(INVALID_EMAILS)(`Should check email: %s is invalid`,
      (invalidEmail) => {
        expect(validateEmail(invalidEmail)).not.toBe(undefined);
      });
  it.each(VALID_PASSWORDS)(`Should check password: %s valid`,
      (validPassword) => {
